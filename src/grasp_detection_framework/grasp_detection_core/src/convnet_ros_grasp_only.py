...
class ConvNetROS:
    def __init__(self) -> None:
        ...
        # self.listener.waitForTransform("myumi_005_base_link", "kinect_master_rgb_camera_link", rospy.Time(0), rospy.Duration(0.1))

        
        camera_info = rospy.wait_for_message("/kinect/rgb/camera_info", CameraInfo, timeout=5)
    ...
    def pixel_to_ee(self, x, y):
        ...
        grasp_point_cam.header.frame_id = "kinect_master_rgb_camera_link"
    ...
    def callback(self, req):
        ...
        depth_image = depth_image.astype(np.uint8) / 1000
        ...
        input_tensor = torch.from_numpy(np.concatenate([rgb_norm, depth_norm], axis=-1)).unsqueeze(0).permute(0, 3, 1, 2).float()
        ...
        grasps = self.post_process_output(pos_output, cos_output, sin_output, width_output, 5)
        ...
        cv2.imwrite("results.png", rgb_image)
